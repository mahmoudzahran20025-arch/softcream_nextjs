...
import { X, Phone, MessageCircle, XCircle, Store, MapPin, Package, Loader2, Edit, AlertCircle } from 'lucide-react'
...
export default function TrackingModal({ isOpen, onClose, order, onEditOrder }: TrackingModalProps) {
  ...
  // ✅ Polling ذكي: سريع في البداية، ثم يبطئ تدريجياً
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      ...
      const now = Date.now()
      // ✅ منع الطلبات المتكررة خلال نفس الثانية
      ...
      try {
        ...
        if (orderData && isMounted) {
          ...
          // ✅ عرض Toast عند تغيير الحالة فقط
          if (oldStatus && oldStatus !== newStatus) {
            ...
            window.dispatchEvent(new CustomEvent('ordersUpdated', { 
              detail: { orderId: order.id, status: newStatus, source: 'polling' } 
            }))
          }

          console.log(`✅ Order status: ${newStatus} (fetch #${pollCount + 1})`)
        }
      } catch (error) {
    ...
    const scheduleNextPoll = () => {
      ...
      // أول 5 طلبات: كل 10 ثواني (أول دقيقة)
      // الـ 5 التالية: كل 15 ثانية (الدقيقة الثانية)
      // بعد ذلك: كل 30 ثانية
      let interval = 30000 // default: 30s
      if (pollCount < 6) interval = 10000      // أول دقيقة: 10s
      else if (pollCount < 11) interval = 15000 // الدقيقة الثانية: 15s
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      ...
      // Trigger event
      window.dispatchEvent(new CustomEvent('ordersUpdated', { 
        detail: { orderId: currentOrder.id, action: 'cancelled', source: 'user' } 
      }))
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border-2 border-blue-200 dark:border-blue-800">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-3">
              {(currentOrder.status === 'pending' || currentOrder.status === 'جديد')
                ? 'سيتم الاتصال بك قريباً لتأكيد الطلب والعنوان' 
                : (currentOrder.status === 'confirmed' || currentOrder.status === 'مؤكد')
